...
int png_save_surface(const char *filename, SDL_Surface *surf)
{
	...
	Uint8* pixeldata = new Uint8[surf->h*surf->pitch];
	memcpy(pixeldata, (Uint8*)surf->pixels, surf->h*surf->pitch);

	for (i = 0; i < surf->h; i++) {
	  row_pointers[i] = (png_bytep)pixeldata+ i*surf->pitch;
	  for (int j = 0; j < surf->w; j++) {
	    Uint8 b = row_pointers[i][j * 3];
	    row_pointers[i][j * 3] = row_pointers[i][j * 3 + 2];
